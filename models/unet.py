...
class Unet3d(Model):
    def __init__(
        ...
        self.transpose_value = out_time // in_time

        # self.init_upsample = nn.ConvTranspose3d(channels, channels, kernel_size=(3,1,1), stride=(transpose_value,1,1))        
    ...
    def forward(
        ...
        time = torch.zeros(x.shape[0]).to(x.device)

        focus_present_mask = default(focus_present_mask, lambda: prob_mask_like((batch,), prob_focus_present, device = device))

        time_rel_pos_bias = self.time_rel_pos_bias(self.out_time, device = x.device)

        # x = self.init_upsample(x)
        x = x.repeat(1, 1, self.out_time // x.size(2), 1, 1) if self.out_time > x.size(2) else x
